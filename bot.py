...
MIN_SECONDS_BETWEEN_REPLIES = int(os.environ.get("MIN_SECONDS_BETWEEN_REPLIES", "30"))
...
def generate_ai_reply(prompt_text):
    resp = openai.ChatCompletion.create(
        model="gpt-4o-mini",
        messages=[
            {"role":"system","content":"You are a friendly social media assistant, short replies (<=120 chars). Avoid abusive content."},
...
def search_and_reply():
    ...
    try:
        ...
        tweets = api.search_tweets(q=query, count=5, result_type='recent', tweet_mode='extended')
        for t in tweets:
            if not can_reply():
                return
            tweet_id = t.id
...
def scheduled_post():
    try:
        text = os.environ.get("SCHEDULE_POST_TEXT", f"Auto post at {datetime.utcnow().isoformat()}")[:280]
